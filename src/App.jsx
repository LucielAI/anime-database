...
function UniverseLinkCard({ data, compact = false }) {
  ...
  return (
    <Link
      to={`/universe/${data.id}`}
      className="group cursor-pointer bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden hover:-translate-y-1 transition-all duration-300 relative flex flex-col focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-400 min-h-[44px]"
    ...
    >
      ...
      <div className="p-4 grow flex flex-col justify-end">
        ...
        <p className="text-[11px] text-gray-300/85 leading-relaxed line-clamp-2 mt-1">{data.tagline}</p>
      </div>
    </Link>
  )
}
    ...
    <div className="min-h-screen bg-[#050508] text-white font-mono selection:bg-cyan-500/30 overflow-x-hidden relative">
      ...
      <section className="max-w-6xl mx-auto px-6 py-10" aria-labelledby="featured-archives-heading">
        ...
        {primaryFeatured && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Link
              to={`/universe/${primaryFeatured.id}`}
              className="lg:col-span-2 rounded-xl border border-white/10 bg-white/5 overflow-hidden hover:border-cyan-400/40 transition-all relative min-h-[220px] md:min-h-[280px]"
            >
              {primaryFeatured.animeImageUrl && (
                <>
                  <img
                    src={primaryFeatured.animeImageUrl}
                    alt={primaryFeatured.anime}
                  ...
                  <img
                    ...
                    alt=""
                    aria-hidden="true"
                    className="absolute -top-8 -right-8 w-2/3 h-2/3 object-cover opacity-15 blur-sm pointer-events-none"
                    loading="eager"
                    decoding="async"
                  />
                </>
              )}
              <div className="absolute inset-0 bg-gradient-to-br from-black/80 via-black/65 to-black/85" />
              <div className="relative z-10 p-6 md:p-8 h-full flex flex-col justify-start">
                <div>
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold uppercase mb-2">{primaryFeatured.anime}</h3>
                  <p className="text-sm md:text-base text-gray-200 leading-relaxed max-w-xl">{primaryFeatured.tagline}</p>
                </div>
                <span className="mt-6 inline-flex items-center gap-2 text-xs text-white uppercase tracking-[0.15em]">Enter archive <ArrowRight className="w-3.5 h-3.5" /></span>
              </div>
            </Link>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-4" aria-labelledby="archive-browse-heading">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                ...
                className={`px-3 py-2 min-h-[44px] rounded-full text-[10px] uppercase tracking-[0.16em] border transition-colors ${sortMode === mode.id ? 'text-white border-cyan-300/60 bg-cyan-400/10' : 'text-gray-400 border-white/10 bg-white/5 hover:text-white'}`}
              >
                {mode.label}
              </button>
            ))}
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link to="/universes" className="px-4 py-2 min-h-[44px] inline-flex items-center rounded-full bg-white/5 border border-white/10 hover:bg-white/10 text-[10px] uppercase tracking-[0.2em] text-gray-300 hover:text-white transition-colors">
      ...
      <footer className="mt-12 pb-10 flex flex-col items-center gap-4 font-mono relative z-10">
        <a href="https://buymeacoffee.com/hashiai" target="_blank" rel="noreferrer" className="group flex items-center gap-2.5 px-5 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-400/30 rounded-full transition-all duration-300">
          <span className="text-[10px] font-bold tracking-[0.2em] text-gray-400 group-hover:text-white transition-colors uppercase">HASHI.AI</span>
...
function UniversesCatalogRoute() {
  ...
  return (
    <main className="min-h-screen bg-[#050508] text-white font-mono px-6 py-14" id="catalog-main">
      <SeoHead {...seo} structuredData={structuredData} />
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
          <label className="md:col-span-2 flex items-center gap-2 border border-white/10 rounded-lg px-3 bg-white/5">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search universe, thesis, or tagline..." className="w-full bg-transparent h-11 text-sm outline-none min-h-[44px]" />
          </label>
          <label className="flex items-center gap-2 border border-white/10 rounded-lg px-3 bg-white/5">
            <ListFilter className="w-4 h-4 text-gray-500" />
            <select value={sortMode} onChange={(e) => setSortMode(e.target.value)} className="w-full bg-transparent h-11 text-sm outline-none min-h-[44px]">
        ...
        {canLoadMore && (
          <div className="mt-6">
            <button onClick={() => setVisibleCount((c) => c + 12)} className="px-4 py-2 min-h-[44px] inline-flex items-center rounded-full border border-white/10 bg-white/5 hover:bg-white/10 text-[10px] tracking-[0.2em] uppercase text-gray-300 hover:text-white">
              Load more
            </button>
          </div>
        )}
